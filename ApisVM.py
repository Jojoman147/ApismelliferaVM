...
class Frontwing(customtkinter.CTkFrame):
    ...
    def analyze(self):
        ...
        if len(body) >= 3:
            ...
            data.append(['Vzdialenosť DO', f'{DO:.4f}'])
        if len(body) >= 3:
        ...
        if len(body) >= 4:
            ...
            data.append(['Dĺžka aR', f'{aR:.4f}'])
            data.append(['Dĺžka bR', f'{bR:.4f}'])
...
To analyse samples within breeds in the Slovak Republic, press
    ...
    def Backexcel(self):
        ...
        # Function to generate a unique file name if 'vysledna tabuľka' already exists
        def generate_unique_filename(base_name, extension):
            counter = 1
            new_name = f"{base_name}.{extension}"
            while os.path.exists(new_name):
